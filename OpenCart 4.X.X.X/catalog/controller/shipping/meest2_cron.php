...
namespace Opencart\Catalog\Controller\Extension\Meest2\Shipping;
 ...
 * Usage: 
 * Add to crontab: 
 * 0 2 * * * curl "https://yourdomain.com/index.php?route=extension/meest2/shipping/meest2_cron.updateAll&key=YOUR_SECRET_KEY"
 * 
 * Or use wget:
 * 0 2 * * * wget -q -O /dev/null "https://yourdomain.com/index.php?route=extension/meest2/shipping/meest2_cron.updateAll&key=YOUR_SECRET_KEY"
 */
class Meest2Cron extends Controller {
    ...
    public function updateAll(): void {
        ...
        $this->load->model('extension/meest2/shipping/meest2');
     ...
     */
    private function importRegions(): array {
        $zoneRegionMap = [
        ...
        foreach ($responseData['result'] as $regionData) {
            ...
            $region = $this->model_extension_meest2_shipping_meest2->getRegion($regionID);
            
            if (empty($region)) {
                $this->model_extension_meest2_shipping_meest2->addRegion($regionDataToSave);
            } else {
                $this->model_extension_meest2_shipping_meest2->editRegion($regionID, $regionDataToSave);
            }
            $count++;
        }
     ...
     */
    private function importCity(): array {
        ...
        $existingCities = $this->model_extension_meest2_shipping_meest2->getAllCities();
        ...
        if (!empty($insertData)) {
            $this->model_extension_meest2_shipping_meest2->bulkInsertCities($insertData);
        }
        
        if (!empty($updateData)) {
            $this->model_extension_meest2_shipping_meest2->bulkUpdateCities($updateData);
        }
    ...
    private function importBranches(): array {
        ...
        $this->model_extension_meest2_shipping_meest2->saveBranchesBatch($responseData['result']);
     ...
     */
    private function importStreets(): array {
        $result = $this->model_extension_meest2_shipping_meest2->importStreets();
     ...
     */
    protected function meestApiV3(string $url, array $data, ?string $token = null, string $method = 'POST'): string {
        if ($token === null) {
     ...
     */
    protected function getValidMeestToken(): string {
        $auth = $this->auth();
